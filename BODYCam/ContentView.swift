...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: displayModeRaw) { _ in
            ...
            applyZoom(zoomFactor, session: captureSession, on: ContentView.sessionQueue) { applied in
                zoomFactor = applied
            }
        }
    ...
    private var background: some View {
        if isFlatTheme {
            if let glow = appTheme.cameraBackgroundGlow,
               displayMode == .saveBattery || displayMode == .pro || displayMode == .circle {
                // Relevant in Save Battery, Pro, and Circle: all three use the
                // same centered card, unlike Normal (fills the screen) or
                // Yapping (shares it with script text), so those are the only
                // modes where a background glow is ever actually visible
                // around it.
    ...
    private var sharedPreviewLayer: some View {
        ...
        GeometryReader { geo in
            ...
            let h: CGFloat = isYapping ? pipSize.height : isNormal
            ...
            let radius: CGFloat = isYapping ? 10 : isNormal ? 0 : isCircle ? previewCircleDiameter / 2 : previewCornerRadius
